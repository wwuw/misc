...
static int __init sunxi_cpufreq_initcall(void)
{
	...
	CPUFREQ_DBG("set ahb apb clock sync\n");

/*
   AXI's parent clk is PLL1 (CPU clk);
*/
//    clk_set_parent(clk_axi,clk_pll);
    clk_set_rate(clk_axi,  clk_get_rate(clk_pll)/2);
/*
   AHB's parent clk is AXI(AXI > 400M);
*/
    clk_set_parent(clk_ahb, clk_axi);
    clk_set_rate(clk_ahb,   clk_get_rate(clk_axi)/2);

/*
    APB's (APB0) parent clk is  AHB;
*/
    clk_set_parent(clk_apb, clk_ahb);
    clk_set_rate(clk_apb,   clk_get_rate(clk_ahb)/2);
/*
    MBUS set at 456M with DCDC3=1.3V
*/
   mod_clk_ops.set_parent(AW_MOD_CLK_MBUS, AW_SYS_CLK_PLL5P);
   mod_clk_ops.set_rate(AW_MOD_CLK_MBUS,  sys_clk_ops.get_rate(AW_SYS_CLK_PLL5P));

	CPUFREQ_DBG("%s: clocks pll=%lu,cpu=%lu,axi=%lu,ahp=%lu,apb=%lu, mbus=%llu\n", __func__,
	       clk_get_rate(clk_pll), clk_get_rate(clk_cpu), clk_get_rate(clk_axi),
	       clk_get_rate(clk_ahb), clk_get_rate(clk_apb), mod_clk_ops.get_rate(AW_MOD_CLK_MBUS));
